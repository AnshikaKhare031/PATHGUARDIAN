...
def fetch_streetlights_by_bbox(bbox: Tuple[float, float, float, float]) -> List[Dict[str, Any]]:
    """Fetches streetlight nodes and lit roads within Kanpur bbox."""
    bbox_str = f"{bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]}"
    query = f"""
    [out:json][timeout:90];
    (
      node["highway"="street_lamp"]({bbox_str});
      way["lit"="yes"]({bbox_str});
    );
    out center;
    """

    logger.info(f"Querying Overpass API for Kanpur lighting data ({bbox_str})...")
    ...
    try:
        ...
        if elements:
            logger.info(f"Retrieved {len(elements)} lighting elements from {OVERPASS_ENDPOINT}.")
            return elements
        else:
...
def process_streetlight_data(raw_nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Processed {len(cleaned_data)} valid lighting points for Kanpur.")
    return cleaned_data
...
def main():
    try:
        raw_nodes = fetch_streetlights_by_bbox(KANPUR_BBOX)
        if not raw_nodes:
            logger.error("No streetlight data retrieved for Kanpur. Terminating execution gracefully.")
            return
